...
## If GitHub main still shows old file content

Sometimes multiple PR iterations on the same files cause messy conflict resolution and old content can remain on `main`.

Use this helper to publish a clean sync branch with only the bootstrap files copied from your working branch:

```bash
bash scripts/sync_pi_bootstrap_to_main.sh work
```

Then open PR from `sync/pi-bootstrap-...` to `main`.
This PR is intentionally small and conflict-resistant.
